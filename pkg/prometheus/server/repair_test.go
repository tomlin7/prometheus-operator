...
func TestResolveStuckStatefulSet_Delete(t *testing.T) {
	...
	// Create stuck pod (old revision, not ready)
	pod := &v1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "test-pod-0",
			Namespace: "default",
...
func TestResolveStuckStatefulSet_IgnoreReady(t *testing.T) {
	...
	// Create ready pod (old revision, but ready - currently checking readiness, maybe logic should skip ready pods?)
	// Logic says: if isPodReady(pod) continue.
	pod := &v1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "test-pod-0",
			Namespace: "default",
...
func TestResolveStuckStatefulSet_IgnoreCurrentRevision(t *testing.T) {
	...
	// Create pod (current revision, not ready)
	// Logic says: if pod.Labels[...] == UpdateRevision continue.
	pod := &v1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "test-pod-0",
			Namespace: "default",
